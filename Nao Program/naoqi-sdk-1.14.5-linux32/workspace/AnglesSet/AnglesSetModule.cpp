...
AnglesSetModule::AnglesSetModule(boost::shared_ptr<AL::ALBroker> broker,
  ...
  , fMemoryFastAccess(boost::shared_ptr<AL::ALMemoryFastAccess>(new AL::ALMemoryFastAccess()))
{
  ...
  BIND_METHOD(AnglesSetModule::startLoop);

  functionName("stopLoop", getName() , "stop");
  BIND_METHOD(AnglesSetModule::stopLoop);

  functionName("setStiffness" , getName(),
                "change stiffness of all joint");
  addParam("value", "new stiffness value from 0.0 to 1.0");
  BIND_METHOD(AnglesSetModule::setStiffness);

  //  start the example.
  //  You can remove it and call AnglesSetModulele.startLoop() to start instead.
  startLoop();
}

AnglesSetModule::~AnglesSetModule()
{
  stopLoop();
}

// Start the example
void AnglesSetModule::startLoop()
{
...
void AnglesSetModule::initFastAccess()
{
  fSensorKeys.clear();
...
void AnglesSetModule::createPositionActuatorAlias()
{
...
void AnglesSetModule::createHardnessActuatorAlias()
{
  AL::ALValue jointAliasses;
...
void AnglesSetModule::preparePositionActuatorCommand()
{
...
void AnglesSetModule::setStiffness(const float &stiffnessValue)
{
...
void AnglesSetModule::connectToDCMloop()
{
  ...
  try
  {
    fDCMPostProcessConnection =
      getParentBroker()->getProxy("DCM")->getModule()->atPostProcess(boost::bind(&AnglesSetModule::motion_control, this));
  }
  catch (const AL::ALError &e)
  {
...
//sensorValues[...] ---> ------.res
//------.cmd ---> command[5][.....][0]
void AnglesSetModule::motion_control()
{
  int DCMtime;
