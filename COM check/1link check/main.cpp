...
#define twidth 0.0001	//sample time
#define TMAX 0.1
...
//	cout << K << endl;
	...
	angles += twidth * K * pseudoInverse((getJacobian(angles)).block<3,1>(0,0)) * Error;
	return angles;
}
...
Vector4d manipulator::COMpos(int start, VectorXd angles)		//start -> joint number
{
	...
	double M_total = M * (1 - start);
	
	for(int i = start; i < angles.size(); i++)
	{
		c = forwardKine(angles, i+1) * c;
...
int main()
{
	ofstream ofs("COMpos.txt");
	Link A;
	manipulator X;	
	
	VectorXd angles(1);
	angles << M_PI/6;
	VectorXd omega = VectorXd::Zero(1);
	
	Vector3d x = TRANS(X.forwardKine(angles,1)); 
	...
	VectorXd dangles;
	VectorXd angles_past = VectorXd::Zero(1);
	...
	{
		xRef << l1 * sin(t/(TMAX)), l1 * cos(t/(TMAX)), 0.0;
		
		Error = xRef - x;
		angles = X.invKine(Error, angles);
		x = TRANS(X.forwardKine(angles,1));
...
//		cout << X.forwardKine(angles) << endl;
		...
		ofs << t << " " << x.transpose() << " " << COMx.transpose() << endl;
	}
/*	
	for(int i = 0; i < 5; i ++)
	{
